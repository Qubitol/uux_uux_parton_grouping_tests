...
#endif
{
  ...
  BridgeKernelBase::BridgeKernelBase( const BufferMomenta& momenta,         // input: momenta
                                      ...
                                      const size_t nevt)
    : MatrixElementKernelBase( momenta, gs, rndhel, rndcol, channelIds, iflavorVec, matrixElements, selhel, selcol )
    , NumberOfEvents( nevt )
...
namespace mg5amcCpu
{
  ...
  BridgeKernelHost::BridgeKernelHost( const BufferMomenta& momenta,         // input: momenta
                                      ...
                                      BufferSelectedColor& selcol,          // output: color selection
                                      const size_t nevt)
    : BridgeKernelBase( momenta, gs, rndhel, rndcol, channelIds, iflavorVec, matrixElements, selhel, selcol, nevt)
  ...
  int BridgeKernelHost::computeGoodHelicities()
  {
    ...
    m_bridge.cpu_sequence( m_fortranMomenta.data(), m_gs.data(), m_rndhel.data(), m_rndcol.data(), pChannelIds, m_iflavorVec.data(), m_matrixElements.data(), m_selhel.data(), m_selcol.data(), goodHelOnly );
...
namespace mg5amcGpu
{
  ...
  BridgeKernelDevice::BridgeKernelDevice( const BufferMomenta& momenta,         // input: momenta
    ...
    : BridgeKernelBase( momenta, gs, rndhel, rndcol, channelIds, iflavorVec, matrixElements, selhel, selcol, gpublocks * gputhreads)
    , m_fortranMomenta( nevt() )
  ...
  int BridgeKernelDevice::computeGoodHelicities()
  {
    constexpr bool goodHelOnly = true;
    constexpr unsigned int* pChannelIds = nullptr; // disable multi-channel for helicity filtering
    m_bridge.gpu_sequence( m_fortranMomenta.data(), m_gs.data(), m_rndhel.data(), m_rndcol.data(), pChannelIds, m_iflavorVec.data(), m_matrixElements.data(), m_selhel.data(), m_selcol.data(), goodHelOnly );
    return m_bridge.nGoodHel();
  }
